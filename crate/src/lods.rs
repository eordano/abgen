...
fn build_lod_bundle(
    ...
    let lod_params = LodBuildParams {
        ...
        vertical_clipping: match (level, meta.vertical_override) {
            (0, _) => [i32::MIN as f64, -(i32::MIN as f64), 0.0, 0.0],
            (_, Some(h)) => [0.0, h, 0.0, 0.0],
            _ => vertical_clipping(meta.parcels.len()),
        },
        // level 0 rolls every object up at its absolute world position, so the
        // prefab root stays at the origin; LOD1 bakes the base-parcel offset.
        root_position: if level == 0 {
            [0.0, 0.0, 0.0]
        } else {
            root_position(meta.base)
        },
